...
# emission model
emi_model <- function(z, sd){
  # select one model of mixtures
  mix_comp <- sample((-1:1), 1)
  
  rnorm(1, z+mix_comp, sd)
}

# transition model
trans_model <- function(z, sd){
  iter <- length(z)
  res <- c()
  
  for(i in 1:iter){
    mix_comp <- sample((0:2), 1)
    res[i] <- rnorm(1, z[i]+mix_comp, sd)
  }
  res
}
...
particle <- function(weight=TRUE, sd=1){
  ...
  for(i in 1:Tsteps){
    ...
    # new particle
    Zn[[i+1]] <- trans_model(Zn[[i]][ind], sd=sd)
  }
...
Even though there is no weighting to the particles, the result of expected locations are pretty nice. 
